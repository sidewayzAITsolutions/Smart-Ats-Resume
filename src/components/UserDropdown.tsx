...
const UserDropdown: React.FC<UserDropdownProps> = ({ userData, className = '' }) => {
  ...
  const userName = userData?.name || userData?.user?.user_metadata?.full_name || null
  ...
  return (
    <div ref={dropdownRef} className={`relative ${className}`}>
      ...
      <button
        ...
        className="flex items-center space-x-2 px-4 py-2 bg-gray-800 hover:bg-orange-800 text-white rounded-lg transition-all duration-200 border border-teal-400 hover:border-purple-600"
        aria-haspopup="true"
      ...
      {/* Dropdown Menu */}
      {isOpen && (
        <div className="absolute right-0 mt-2 w-64 bg-gray-800 rounded-xl shadow-2xl border border-gray-700 overflow-hidden animate-in fade-in slide-in-from-top-1 duration-200">
          {/* User Info Header */}
